...

{
  ...
  if [[ "$ONLY_VIDEO" == true ]]; then
    ...
    ffmpeg <./stop -y -f alsa -i pulse -f x11grab -draw_mouse 0 -framerate $FRAMERATE -video_size $RESOLUTION -i :$DISPLAY_NUM -c:a aac -c:v libx264 -preset ultrafast -crf 28 -refs 4 -qmin 4 -pix_fmt yuv420p -filter:v "fps=$FRAMERATE,drawtext=text='IMAGE BY NAEVATEC v1.0':fontsize=50:fontcolor=white:x=(w-text_w)/2:y=h-text_h-20:shadowcolor=black:shadowx=2:shadowy=2" "/recordings/$VIDEO_ID/$VIDEO_NAME.$VIDEO_FORMAT"
  fi
